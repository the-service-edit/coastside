...
(function(){
  ...
  /* ---------- V6 split home: photo window follows the section you are reading ---------- */
  var chapters=d.querySelectorAll('.oh-chapter[data-scene]');
  if(chapters.length&&'IntersectionObserver' in w){
    var wide=w.matchMedia('(min-width: 701px)');
    var st=d.getElementById('scene-title'),sl=d.getElementById('scene-label');
    var sceneIO=new IntersectionObserver(function(es){
      if(!wide.matches)return;
      es.forEach(function(e){
        if(!e.isIntersecting)return;var s=e.target;
        d.querySelectorAll('.oh-img').forEach(function(im){im.classList.toggle('is-visible',im.dataset.image===s.dataset.scene)});
        if(st)st.innerHTML=s.dataset.title;if(sl)sl.textContent=s.dataset.label;
        d.querySelectorAll('.oh-rail a').forEach(function(a){a.setAttribute('aria-current',String(a.hash==='#'+s.id))});
      });
    },{rootMargin:'-25% 0px -55% 0px'});
    chapters.forEach(function(ch){sceneIO.observe(ch)});
  }
  var topic=d.getElementById('oh-topic'),go=d.getElementById('oh-go');
  if(topic&&go){var base=go.getAttribute('href');topic.addEventListener('change',function(){go.setAttribute('href',base+(topic.value?'?services='+encodeURIComponent(topic.value):''))})}
